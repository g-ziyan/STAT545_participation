...
```{r}
expect_equal(0.1 + 0.2, 0.3, 0.3)
expect_identical(0.3, 0.3)
```
...
test_that("Simple cases work", {
    ...
    expect_lt(max_minus_min(runif(100,min=0,max=1)),1)
})
...
`test_that` used outside the function and `stop` used inside the function