...
tibble(let = letters, 
       ...
       comb = paste(let,num,sep="!")) #`paste0` has no space here
```
...
gather(lotr_untidy, key= "Gender", value="Word", Female: Male)
gather(lotr_untidy, key= "Gender", value="Word", contains("ale")) #female&male
```
...
lotr_list
do.call(bind_rows, lotr_list)#(function,list)
```
